...
def load_and_preprocess_data(file_path):
    ...
    df.columns = ["ds", "y", "day", "temperature_2m_mean", "precipitation_sum_mm", "event_score"]
...
def prepare_future_dataframe(model, df, day_dummies, prediction_days=365):
    ...
    if 'event_score' in df.columns:
        future['event_score'] = df['event_score'].reindex(future.index, fill_value=4)  # Default to 4 for unknown events
    else:
        raise ValueError("event_score data missing for predictions")
...
if __name__ == "__main__":
    ...
    df, day_dummies = load_and_preprocess_data(os.path.join(base_path, "data", "sales_weather_and_score_data.csv"))
    ...
    create_visualization(model, forecast, os.path.join(base_path, "outputs", "forecast_plot.html"))
    export_results(forecast, os.path.join(base_path, "outputs", "forecast_results.csv"))
